...
app.get('/tutores', async (req, res)=>{
    ...
    const [rows] = await pool.query('SELECT    u.name,    u.pic_url,    TRUNCATE(AVG(s.stars), 1) AS rating, COUNT(u.name) AS nreviews  FROM    user u    INNER JOIN session s ON u.id = s.id_tutor  GROUP BY    u.name,    u.pic_url  HAVING    COUNT(s.id) >= 1  ORDER BY    rating DESC')
    res.json(rows)    
})
